...
在手机上，个人信息区域会随着滚动逐步收缩成统一顶栏。紧凑状态保留头像、网站名称、深浅色切换、语言切换和菜单入口，同时让页面变化保持连续。
...
文章还可以选择声明人工写作或 AIGC 辅助写作标识。选择后的双语标签会与文章操作区一同显示；未设置该字段的文章继续使用原来的页脚。
...
另一个使用虚构数据的产品目录展示搜索、状态筛选、排序、分类折叠和响应式卡片等数据驱动页面能力。它与启动台相互独立，不需要时可以直接移除。
...
在触控设备上，浮动章节导航通过点击展开，选择章节或点击外部后自动收起，使长页面保持易用，又不会长期遮挡阅读区域。
...
使用兼容后端的网站还可以启用文档所述的上传会话与评论验证接口。它们默认关闭，避免模板在后端尚未实现验证时造成已经具备安全检查的误解。
...
模板提供 Web App Manifest、安装图标、Apple 独立窗口元数据和浏览器主题色，构成添加到主屏幕所需的基础体验；目前不启用离线缓存。
